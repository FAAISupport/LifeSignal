...
export async function confirmFaithSignalyCheckin(input: {
  FaithSignalyCheckinId: string;
  actorProfileId?: string;
  ...
  const { data, error } = await supabase
    .from("FaithSignaly_checkins")
    .update({ status: "responded", responded_at: now, response_channel: input.responseChannel })
    .eq("id", input.FaithSignalyCheckinId)
    .select("id, monitored_person_id")
    .maybeSingle();

  if (error || !data) {
    throw new Error(`FaithSignaly_confirm_failed:${error?.message}`);
  }

  await writeAuditLog({
    actorProfileId: input.actorProfileId ?? null,
    action: "FaithSignaly.confirmed",
    entityType: "FaithSignaly_checkins",
    entityId: data.id,
