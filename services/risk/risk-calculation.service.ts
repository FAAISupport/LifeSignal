  ...
  for (const person of people ?? []) {
    ...
    const [{ data: c7 }, { data: c30 }, { data: med7 }, { data: rec7 }, { count: interventions30 }, { count: emergencies30 }] =
      await Promise.all([
        ...
        supabase.from("FaithSignaly_checkins").select("status").eq("monitored_person_id", person.id).gte("expected_at", since7),
    ...
    const factors = {
      ...
      FaithSignaly_miss_7d: Math.round(percent(recMiss7, rec7?.length ?? 0) * 0.08),
      ...
      .upsert(
        {
          ...
          FaithSignaly_miss_rate_7d: percent(recMiss7, rec7?.length ?? 0),
