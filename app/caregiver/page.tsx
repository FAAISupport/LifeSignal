        ...
        {data.cards.map((card) => (
          <section key={String(card.monitoredPersonId)} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between">
              ...
              <span className={`rounded-full px-3 py-1 text-base font-semibold ${riskBadge(String(card.riskLevel))}`}>
                {String(card.riskLevel)}
              </span>
            </div>

            <p className="mt-3 text-lg text-slate-600">Latest check-in: {String(card.latestCheckinStatus)}</p>
            <p className="mt-1 text-lg text-slate-600">Trend: {String(card.trend)}</p>
            <p className="mt-3 text-lg text-slate-700">{String(card.explanation)}</p>

            <div className="mt-3">
              <h3 className="text-base font-semibold uppercase text-slate-500">Top contributing reasons</h3>
              <ul className="mt-1 list-disc pl-5 text-lg text-slate-700">
